...
```tsx
const calendarOptions = useSelector((state: RootState) => calendarProjectOptionsSelector(state, projectId));
```
...
```tsx
const groupedPackages = useSelector((state: RootState) => groupedPackagesSelector(state, { ownerId, projectId }));
```
